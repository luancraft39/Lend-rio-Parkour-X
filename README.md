...
<script>
    ...
    // SISTEMA DE ESCALA (ZOOM)
    let gameScale = 1;
    function resize() { 
        canvas.width = window.innerWidth; 
        canvas.height = window.innerHeight;
        // Se a tela for pequena (celular), diminui a escala para 0.7 (zoom out)
        gameScale = window.innerWidth < 800 ? 0.7 : 1;
    }
    ...
    jumpBtn.addEventListener("pointerdown", (e) => {
        e.preventDefault();
        startMusic(); // Tenta tocar música ao interagir
    ...
    function resetMap() { 
        platforms = []; stars = []; 
        lastY = (canvas.height / gameScale) * 0.6; 
    ...
    function update() {
        ...
        camera.x += ((player.x - (canvas.width / gameScale) / 2.5) - camera.x) * 0.1;
        camera.y += ((player.y - (canvas.height / gameScale) / 2) - camera.y) * 0.1; 
        
        if(lastX < player.x + (canvas.width / gameScale)) { 
            let gap = 170 + Math.random() * 80; let pW = 250 + Math.random() * 100; 
            lastY = Math.max((canvas.height / gameScale) * 0.3, Math.min((canvas.height / gameScale) * 0.7, lastY + (Math.random()-0.5) * 200)); 
            platforms.push({ x: lastX + gap, y: lastY, w: pW, h: 600 }); 
            if(Math.random() > 0.4) stars.push({ x: lastX + gap + pW/2, y: lastY - 60, collected: false });
            lastX += gap + pW; 
        }
        if(player.y > camera.y + (canvas.height / gameScale) + 150) { 
    ...
    function draw() {
        ...
        ctx.save(); 
        ctx.scale(gameScale, gameScale); // APLICA O ZOOM
