...
<head>
    ...
    <style>
        ...
        .menu-box {
            ...
            display: flex; flex-direction: column; align-items: center; /* Garante alinhamento vertical */
        }
        ...
        .btn-group {
            ...
            gap: 12px;
            width: 100%; /* Ocupa a largura da menu-box */
            align-items: center;
        }
        ...
        #joystick-area { 
            ...
            border: 4px solid rgba(255,255,255,0.4); backdrop-filter: blur(4px); touch-action: none;
        }
        #joystick-stick { position: absolute; top: 40px; left: 40px; width: 60px; height: 60px; background: #fff; border-radius: 50%; box-shadow: 0 4px 10px rgba(0,0,0,0.3); pointer-events: none; }

        #jump-button { 
            position: absolute; bottom: 45px; right: 45px; width: 120px; height: 120px; 
            background: rgba(255,255,255,0.25); border: 5px solid #fff; border-radius: 50%; 
            color: #fff; display: flex; align-items: center; justify-content: center; 
            font-weight: 900; pointer-events: auto; font-size: 22px; touch-action: none;
            box-shadow: 0 6px 15px rgba(0,0,0,0.2); text-shadow: 1px 1px 3px rgba(0,0,0,0.5);
        }
        #jump-button:active { background: rgba(255,255,255,0.5); transform: scale(0.92); }
...
<script>
    // Todo o seu JavaScript original continua aqui intacto
    ...
    function updateJoy(e) {
        ...
        joystick.x = normX;
        joyStick.style.left = (40 + normX * 35) + "px";
    }
    ...
    function continueGame() { document.getElementById("death").style.display = "none"; player.y = lastY - 100; player.vx = 0; player.vy = 0; gameState = 'playing'; }
    ...
    function update() {
        ...
        camera.y += ((player.y - canvas.height/1.8) - camera.y) * 0.1;
        
        if(lastX < player.x + canvas.width) { 
            let gap = 170 + Math.random() * 80; let pW = 250 + Math.random() * 100; 
            lastY = Math.max(canvas.height*0.2, Math.min(canvas.height*1.5, lastY + (Math.random()-0.5)*250)); 
            platforms.push({ x: lastX + gap, y: lastY, w: pW, h: 600 }); 
            if(Math.random() > 0.4) stars.push({ x: lastX + gap + pW/2, y: lastY - 60, collected: false });
            lastX += gap + pW; 
        }
        if(player.y > camera.y + canvas.height + 300) { if(gameState === 'playing') { gameState = 'paused'; lives--; updateHearts(); if(lives <= 0) resetToMenu(); else document.getElementById("death").style.display = "flex"; } else { player.y = -100; player.x += 100; } }
    }
