...
<script>
    ...
    function resize() { 
        ...
        // Ajuste de escala para celular
        gameScale = window.innerWidth < 800 ? 0.8 : 1;
    }
    ...
    function update() {
        ...
        // AJUSTE DA CÂMERA: NO CELULAR, ELA FICA MAIS BAIXA PARA VER O CHÃO
        let camOffsetY = (window.innerWidth < 800) ? 1.5 : 2; 
        camera.x += ((player.x - (canvas.width / gameScale) / 2.5) - camera.x) * 0.1;
        camera.y += ((player.y - (canvas.height / gameScale) / camOffsetY) - camera.y) * 0.1; 
        ...
        if(player.y > camera.y + (canvas.height / gameScale) + 200) { 
