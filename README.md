...
<head>
    ...
    <style>
        ...
        /* AJUSTE NA CAIXA DO MENU PARA NÃO SAIR DA TELA */
        .menu-box {
...
<script>
    ...
    /* CORREÇÃO DO MAPA ESCONDIDO NO CELULAR */
    function resetMap() { 
        platforms = []; stars = []; 
        lastY = canvas.height * 0.6; // Começa o chão em 60% da altura para ficar visível
    ...
    function update() {
        ...
        /* AJUSTE DA CÂMERA PARA NÃO ESCONDER O PERSONAGEM */
        camera.x += ((player.x - canvas.width / 2.5) - camera.x) * 0.1;
        camera.y += ((player.y - canvas.height / 2) - camera.y) * 0.1; // Centraliza verticalmente
        
        if(lastX < player.x + canvas.width) { 
            let gap = 170 + Math.random() * 80; let pW = 250 + Math.random() * 100; 
            // Limita a variação do chão para ficar na zona central
            lastY = Math.max(canvas.height * 0.3, Math.min(canvas.height * 0.7, lastY + (Math.random()-0.5) * 200)); 
        ...
        if(player.y > camera.y + canvas.height + 150) { 
            if(gameState === 'playing') { 
                ...
                player.y = lastY - 100; player.x -= 200; 
            } 
        }
    }
